...
from constants import Height, EditingPanelSize, RepLocation
class Sprite():
    def __init__(self, pixelSize, pixelsInSprite):
        self.pixelArray = []
        self.pixelSize = pixelSize
        self.pixelsInSprite = pixelsInSprite
        self.blockSize = (EditingPanelSize[0]/self.pixelsInSprite[0], EditingPanelSize[1]/self.pixelsInSprite[1])
    ...
    def drawSpriteRepresentation(self, window):
        ...
        pygame.draw.rect(window, (0, 0, 0), (RepLocation[0]-1, RepLocation[1]-1, xx+2,yy+2))
        for x in range(self.pixelsInSprite[0]):
            for y in range(self.pixelsInSprite[1]):
    ...
    def separatePixels(self, window):
        for x in range(1, self.pixelsInSprite[0] + 1):
            pygame.draw.line(window, (0, 0, 0), (x*self.blockSize[0], 0), (x*self.blockSize[0], Height))
